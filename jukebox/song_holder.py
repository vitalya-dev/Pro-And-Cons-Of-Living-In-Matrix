...
class SongHolder(Shape):
  ...
  def _calculate_position_for_new_song_entry(self):
    if len(self._song_entries) > 0:
      song_entry_position = tuple_math(self._song_entries[-1].world_space_rect.bottomleft, '+', (0, self._space_between_song_entries))
  ...
  def _song_entries_total_height(self):
    if len(self._song_entries) > 0:
      song_entries_total_height = sum([song_entry.world_space_rect.height for song_entry in self._song_entries])
  ...
  def draw(self):
    ...
    for song_entry in self._song_entries:
      self._surface.blit(song_entry.draw(), song_entry.world_space_rect.topleft)
