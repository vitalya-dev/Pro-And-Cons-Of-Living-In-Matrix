...
// Accessors
struct json_value_s *json_get(struct json_object_s *obj, const char *name) {
  for (struct json_object_element_s *elem = obj->start; elem; elem = elem->next) {
    if (!strcmp(elem->name->string, name)) {
      return elem->value;
    }
  }
  return NULL;
...
struct json_value_s *json_at(struct json_array_s *arr, size_t idx) {
  assert(idx < arr->length && "JSON: Output bounds");
  size_t i = 0;
  for (struct json_array_element_s *elem = arr->start; elem; elem = elem->next) {
    if (i == idx) {
      return elem->value;
    }
    i++;
...
int json_try_string(struct json_value_s *v, const char **o) {
  if (v->type != json_type_string) return 0;
  *o = ((struct json_string_s *)v->payload)->string;
  return 1;
}
int json_try_object(struct json_value_s *v, struct json_object_s **o) {
  if (v->type != json_type_object) return 0;
  *o = (struct json_object_s *)v->payload;
  return 1;
}
int json_try_array(struct json_value_s *v, struct json_array_s **o) {
  if (v->type != json_type_array) return 0;
  *o = (struct json_array_s *)v->payload;
  return 1;
}
int json_try_number(struct json_value_s *v, double *o) {
  if (v->type != json_type_number) return 0;
  *o = atof(((struct json_number_s *)(v->payload))->number);
  return 1;
}

const char *json_as_string(struct json_value_s *v) {
  assert(v->type == json_type_string && "JSON: Not string");
  return ((struct json_string_s *)v->payload)->string;
}
struct json_object_s *json_as_object(struct json_value_s *v) {
  assert(v->type == json_type_object &&"JSON: Not object");
  return (struct json_object_s *)v->payload;
}
struct json_array_s *json_as_array(struct json_value_s *v) {
  assert(v->type == json_type_array && "JSON: Not array");
  return (struct json_array_s *)v->payload;
}
double json_as_number(struct json_value_s *v) {
  assert(v->type == json_type_number && "JSON: Not number");
  return atof(((struct json_number_s *)(v->payload))->number);
}
