...
class SongSelector(Shape):
  def __init__(self, size=SCREEN_SIZE, background_color=BLACK, parent=None):
    super().__init__()
    #================#
    self.on_toggle = []
    #================#
  ...
  def _layout_elements(self):
    self._letters_first_row.pivot = (0.5, 0)
    self._letters_first_row.position = self.self_space_rect.midtop
    #================#
    self._letters_second_row.pivot = (0.5, 1)
    self._letters_second_row.position = self.self_space_rect.midbottom
...
if __name__ == '__main__':
  ...
  #================#
  song_selector = SongSelector(size=tuple_math(SCREEN_SIZE, '*', (0.8, 0.2)))
